...
-- Create trigger for private_leaderboards updated_at
...
CREATE POLICY "Users can view others' block status for invite checks"
  ON user_preferences
  FOR SELECT
  USING (true); -- Anyone can check if someone blocks invites (needed for addMemberToLeaderboard)

-- Create a secure view that only exposes public profile data for leaderboards
-- This prevents users from accessing email addresses even if they can query the profiles table
-- Note: The view is updated in migration 002 to include normalized columns (total_xp, day_streak, etc.)
-- See migration 002_normalize_stats_columns.sql for the full view definition

-- Grant access to authenticated users
GRANT SELECT ON public_profile_data TO authenticated;

-- Note: We use a view instead of a permissive RLS policy to prevent column-level access
-- This ensures users can only access the columns exposed by the view, not email or other sensitive data
